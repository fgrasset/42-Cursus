...
#include <fcntl.h> // Include fcntl for non-blocking mode
...
int main(int ac, char **av) {
    ...
    servaddr.sin_addr.s_addr = htonl(INADDR_LOOPBACK); // Use INADDR_LOOPBACK for 127.0.0.1
    ...
    while (1) {
        ...
        for (int fd = 0; fd <= max; fd++) {
            ...
            if (FD_ISSET(fd, &active) && fd != listeningSocket) {
                int read_bytes = recv(fd, bufRead, sizeof(bufRead), 0);
                if (read_bytes <= 0) {
